...
import (
	...
	"main.go/plugins/world_mirror/server/cmd"
	"main.go/plugins/world_mirror/server/internal"
	_ "main.go/plugins/world_mirror/server/item" // Imported for compiler directives.
	"main.go/plugins/world_mirror/server/player"
	"main.go/plugins/world_mirror/server/player/playerdb"
	"main.go/plugins/world_mirror/server/session"
	"main.go/plugins/world_mirror/server/world"
	"main.go/plugins/world_mirror/server/world/biome"
	"main.go/plugins/world_mirror/server/world/generator"
)
...
// behaviour.
func New(c *Config, log internal.Logger, injectFns *InjectFns) *Server {
	if log == nil {
		log = logrus.New()
	}
...
type NonEditingGameMode struct {
}

func (n NonEditingGameMode) AllowsEditing() bool {
	return true
}

func (n NonEditingGameMode) AllowsTakingDamage() bool {
	return true
}

func (n NonEditingGameMode) CreativeInventory() bool {
	return false
}

func (n NonEditingGameMode) HasCollision() bool {
	return true
}

func (n NonEditingGameMode) AllowsFlying() bool {
	return false
}

func (n NonEditingGameMode) AllowsInteraction() bool {
	return true
}

func (n NonEditingGameMode) Visible() bool {
	return true
}
...
func (server *Server) finaliseConn(ctx context.Context, conn session.Conn, l Listener, wg *sync.WaitGroup) {
	...
	NetEaseGameData := server.InjectFns.GetNetEaseGameData()
	//NetEaseClinetData := server.InjectFns.GetNetEaseClientData()
	NetEaseIdentityData := server.InjectFns.GetNetEaseIdentityData()

	GameRules := make([]protocol.GameRule, len(NetEaseGameData.GameRules))
	for i, r := range NetEaseGameData.GameRules {
		GameRules[i] = protocol.GameRule(r)
	}

	pos := server.InjectFns.GetPos()
	reflectGameData := minecraft.GameData{
		WorldName:                    NetEaseGameData.WorldName,
		Difficulty:                   NetEaseGameData.Difficulty,
		EntityUniqueID:               NetEaseGameData.EntityUniqueID,
		EntityRuntimeID:              NetEaseGameData.EntityRuntimeID,
		PlayerGameMode:               NetEaseGameData.PlayerGameMode,
		BaseGameVersion:              "",
		PlayerPosition:               pos,
		Pitch:                        NetEaseGameData.Pitch,
		Yaw:                          NetEaseGameData.Yaw,
		Dimension:                    NetEaseGameData.Dimension,
		WorldSpawn:                   protocol.BlockPos(NetEaseGameData.WorldSpawn),
		WorldGameMode:                NetEaseGameData.WorldGameMode,
		GameRules:                    GameRules,
		Time:                         NetEaseGameData.Time,
		ServerBlockStateChecksum:     0,
		CustomBlocks:                 nil,
		Items:                        server.itemEntries(),
		PlayerMovementSettings:       protocol.PlayerMovementSettings(NetEaseGameData.PlayerMovementSettings),
		ServerAuthoritativeInventory: NetEaseGameData.ServerAuthoritativeInventory,
		Experiments:                  nil,
	}
	...
	playerData := &player.Data{
		...
		XPPercentage:    0,
		XPSeed:          0,
		GameMode:        NonEditingGameMode{},
		Inventory:       player.InventoryData{},
		Effects:         nil,
		FireTicks:       0,
		FallDistance:    0,
		Dimension:       0,
	...
	if err := conn.StartGameContext(ctx, reflectGameData); err != nil {
...
func (server *Server) createPlayer(id uuid.UUID, conn session.Conn, data *player.Data) *player.Player {
	...
	s := session.New(conn, server.c.Players.MaximumChunkRadius, server.log, &server.joinMessage, &server.quitMessage, server.InjectFns.SessionInjectFns)
...
func (server *Server) createWorld(d world.Dimension, biome world.Biome, layers []world.Block, s *world.Settings) *world.World {
	...
	p, err := server.InjectFns.GetProvider()
	if err != nil {
