...
func InitFromReflectPalettedStorage(src *reflect_chunk.PalettedStorage) *PalettedStorage {
	return &PalettedStorage{
		bitsPerIndex:       src.GetBits(),
		filledBitsPerIndex: src.GetfilledBitsPerIndex(),
		indexMask:          src.GetindexMask(),
		indicesStart:       src.GetindicesStart(),
		palette:            InitFromReflectPalette(src.Getpalette()),
