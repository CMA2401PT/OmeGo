...
import reflect_chunk "main.go/plugins/chunk_mirror/server/world/chunk"
...
func InitFromReflect(src *reflect_chunk.SubChunk) *SubChunk {
	storages := make([]*PalettedStorage, 0)
	for _, s := range src.GetStorages() {
		storages = append(storages, InitFromReflectPalettedStorage(s))
	}
	return &SubChunk{air: src.GetAir(), Storages: storages, blockLight: src.GetBlockLight(), skyLight: src.GetSkyLight()}
}
