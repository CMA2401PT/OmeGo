...
func mapArt(path string, MapX, MapY, MapZ int, blockFn func(X, Y, Z int, blockName string, blockData int)) error {
	if MapY != 0 {
	...
	if MapY == 0 {
		...
		for Z, row := range blockImg {
			for X, blk := range row {
				blockFn(X, 0, Z, blk.Name, blk.Meta)
			}
		}
	} else {
		...
		for Z, row := range blockImg {
			for X, blk := range row {
				blockFn(X, YMap[Z][X], Z, blk.Name, blk.Meta)
			}
		}
	}
