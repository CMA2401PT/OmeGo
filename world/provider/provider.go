...
type LoadChunkInterceptorFn func(position world.ChunkPos) (c *chunk.Chunk, exists bool, err error)
...
func (p *Provider) InitFromFile(dir string) error {
	...
	if err != nil {
		return fmt.Errorf("error opening leveldb database: %w", err)
	}
	p.db = db
	return nil
}
...
func (p *Provider) Close() error {
	...
	f, err := os.OpenFile(filepath.Join(p.saveDir, "level.dat"), os.O_TRUNC|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		return fmt.Errorf("error opening level.dat file: %w", err)
	}
	...
	if err := ioutil.WriteFile(filepath.Join(p.saveDir, "levelname.txt"), []byte(p.d.LevelName), 0644); err != nil {
