...
func Dial(config *LoginToken) (*Conn, error) {
	var d Dialer
	return d.Dial(config)
}
...
func DialTimeout(config *LoginToken, timeout time.Duration) (*Conn, error) {
	var d Dialer
	return d.DialTimeout(config, timeout)
}
...
func DialContext(ctx context.Context, config *LoginToken) (*Conn, error) {
	var d Dialer
	return d.DialContext(ctx, config)
}

// Dial dials a Minecraft connection to the address passed over the network passed. The network is typically
// "raknet". A Conn is returned which may be used to receive packets from and send packets to.
func (d Dialer) Dial(token *LoginToken) (*Conn, error) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*30)
	defer cancel()
	return d.DialContext(ctx, token)
}

// DialTimeout dials a Minecraft connection to the address passed over the network passed. The network is
// typically "raknet". A Conn is returned which may be used to receive packets from and send packets to.
// If a connection is not established before the timeout ends, DialTimeout returns an error.
func (d Dialer) DialTimeout(token *LoginToken, timeout time.Duration) (*Conn, error) {
...
func (d Dialer) DialContext(ctx context.Context, token *LoginToken) (conn *Conn, err error) {
	address := token.Address
	key := token.PrivateKey
	chainData := token.UserToken
